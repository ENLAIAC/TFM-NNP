...
DeepMD_MODEL_FILES=("graph_3_006_compressed.pb" "graph_1_006_compressed.pb" "graph_2_006_compressed.pb")
LAMMPS_IN_FILE="ppimg2-hydrolysis.in"
LAMMPS_LOG_FILE="ppimg2-hydrolysis.log"
LAMMPS_OUT_FILE="ppimg2-hydrolysis.out"
EXTRA_FILES=("ppimg2-hydrolysis.lmp" "plumed_ppimg2-hydrolysis.dat")

SCRATCH=/gpfs/scratch/uv36/elia/JOBS
...
if [ "${SLURM_JOB_PARTITION}" == "acc" ]; then # Example to use the DeepMD_MODEL_VERSION variable
...
elif [ "${SLURM_JOB_PARTITION}" == "gpp" ]; then
echo "GPU on CPU partition??? Hell nah, aborting..."; exit 1
else
echo "There is no ${SLURM_JOB_PARTITION}. Aborting..."; exit 1;
fi

#Check if the lammps executable is available
LAMMPS_EXE=$(command -v lmp_mpi) || ( echo "Executable (lmp) not found. Aborting..." ; exit 1 )
...
LAUNCH_CMD="${SRUN_LAMMPS_EXE} -in ${LAMMPS_IN_FILE} -log ${LAMMPS_LOG_FILE} -screen none"
