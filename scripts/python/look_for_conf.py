...
parser.add_argument('--file', '-f', dest='COLVAR', type=str, help='MANDATORY - Provide the name of the file to look through')
...
def main():
    if len(sys.argv)==1:
        parser.print_help()
        sys.exit()
    elif COLVAR_file is None:
        print("WARNING: No file name provided")
        parser.print_help()
        sys.exit()
    idxs=idx_search()
    find_conf(idxs)
