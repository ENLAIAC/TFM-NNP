...
#-SBATCH --nodes 1
#SBATCH --ntasks 1 #TOCHECK IF THE TASK IS ACTUALLY ONE
...
array_line=$(sed -n "${SLURM_ARRAY_TASK_ID_LINE}p" "job-array-params_lammps-deepmd_explore_cpu_glogin2.lst")
...
if [ "${SLURM_JOB_QOS}" == "gpp" ]; then # Example to use the DeepMD_MODEL_VERSION variable
        if [ "${DeepMD_MODEL_VERSION}" == "3.g" ]; then
                module purge
